...
def _as_pair(y_true, y_pred) -> tuple[np.ndarray, np.ndarray]:
    ...
    Emptiness is checked here rather than in :func:`as_1d` so the message can
    name both series at once.
    """
    true = as_1d("y_true", y_true, allow_empty=True)
    pred = as_1d("y_pred", y_pred, allow_empty=True)
